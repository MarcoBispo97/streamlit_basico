...
recebita_estados = dados.drop_duplicates(subset='Local da compra')[['Local da compra', 'lat', 'lon']].merge(
...
receita_categorias = dados.groupby('Categoria do Produto')[
    'Preço'].sum().sort_values(ascending=False)
...
# Graficos
fig_mapa_receita = px.scatter_geo(recebita_estados,
                                  lat='lat',
                                  lon='lon',
                                  scope='south america',
                                  size='Preço',
...
receita_estados = receita_estados.sort_values(
    by='Preço', ascending=True).head(10)
receita_estados.reset_index(inplace=True)
fig_receita_estados = px.bar(
    receita_estados.head(10),
    x=receita_estados.iloc[:, 0],
    y=receita_estados.iloc[:, 1],
    text_auto=True,
